      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-3' : 'bg-white py-4'}`}>
        <div className="container mx-auto px-4 flex items-center">
          ...
          <div className="flex-1 lg:flex-none">
            <NavLink to="/" className="group shrink-0 block w-fit">
              <Logo />
            </NavLink>
          </div>
          ...
          <nav className="hidden lg:flex flex-1 items-center justify-center gap-1">
            {navItems.map((item) => (
              <div key={item.name} className="relative group">
                ...
                {item.subItems && (
                  <div className="absolute top-full left-0 pt-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 -translate-y-1 transition-all duration-300 z-50">
                    <div className="bg-white border border-slate-100 p-1.5 rounded-sm shadow-[0_20px_50px_rgba(0,0,0,0.06)] min-w-[240px]">
                      <div className="relative z-10 space-y-0.5">
                        {item.subItems.map((sub) => (
                          <NavLink 
                            ...
                            to={sub.path}
                            className={({ isActive }) => `
                              block py-3 px-5 text-[9px] font-black uppercase tracking-[0.2em] transition-all rounded-sm
                              ${isActive ? 'bg-brand-blue/5 text-brand-blue' : 'text-slate-500 hover:text-brand-dark hover:bg-slate-50'}
                            `}
                          >
                            {sub.name}
                          </NavLink>
          ...
          <div className="flex items-center justify-end flex-1 lg:flex-none">
            ...
            <button className="lg:hidden text-brand-dark p-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? <X /> : <Menu />}
            </button>
          </div>
        </div>
